"""
BT-SRAC: Bradley-Terry State-Ranking Actor-Critic
================================================
A modular, problem-agnostic implementation of the BT-SRAC algorithm
"""
...
class BTSRAC:
    def __init__(self, env: RMABEnv, config: Optional[BTSRACConfig] = None):
        self.env = env
        self.cfg = config or BTSRACConfig()
        cfg = self.cfg
    ...
    def train(self, num_episodes: Optional[int] = None,
        ...
        for ep in range(n_ep):
            ...
            if test_seeds is not None and (ep + 1) % cfg.test_every == 0:
                ...
                print(f"   [Test] BT-SRAC: {test_reward:.2f} | Whittle: {whittle_baseline if whittle_baseline else 'N/A'}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("BT-SRAC — Deadline Scheduling (Local Plotting)")
    print("=" * 60)
    ...
    for exp_N, exp_M in experiments:
        ...
        cfg = BTSRACConfig(num_episodes=30000, test_every=50, log_every=50, device=device)
        ...
        agent = BTSRAC(DeadlineSchedulingEnv(**env_kwargs), cfg)
        ...
        plt.plot(eps, rewards, label='BT-SRAC (Test Reward)', marker='o', markersize=4)
