"""
BT-PPO-SRAC: Bradley-Terry State-Ranking Actor-Critic with PPO Clipping
================================================
A modular, problem-agnostic implementation of the BT-SRAC algorithm with PPO stabilization.
"""
...
class BT_PPO_SRAC:
    def __init__(self, env: RMABEnv, config: Optional[BT_PPOConfig] = None):
        self.env = env
        self.cfg = config or BT_PPOConfig()
        cfg = self.cfg
        ...
        # Old actor for PPO clipping
        self.old_actor = Actor(sdim, cfg.actor_hidden).to(self.device)
    ...
    def train(self, num_episodes: Optional[int] = None,
        ...
        for ep in range(n_ep):
            ...
            for _ in range(cfg.episode_len):
                ...
                # Actor Update (PPO-Clipped Bradley-Terry Pairwise Comparison)
                h_diff = arm_logits.unsqueeze(1) - arm_logits.unsqueeze(0)
                p_ij = torch.sigmoid(lam * h_diff)
                
                with torch.no_grad():
                    old_logits = self.old_actor(self._ss_enc)
                    old_arm_logits = self._arm_logits(states, old_logits)
                    old_h_diff = old_arm_logits.unsqueeze(1) - old_arm_logits.unsqueeze(0)
                    old_p_ij = torch.sigmoid(lam * old_h_diff)
                
                adv_diff = advantage.unsqueeze(1) - advantage.unsqueeze(0)
                mask = (adv_diff > 0).float()
                
                ratio = p_ij / (old_p_ij + 1e-8)
                surr1 = ratio * mask
                surr2 = torch.clamp(ratio, 1.0 - cfg.ppo_epsilon, 1.0 + cfg.ppo_epsilon) * mask
            ...
            if test_seeds is not None and (ep + 1) % cfg.test_every == 0:
                ...
                print(f"   [Test] BT-PPO-SRAC: {test_reward:.2f} | Whittle: {whittle_baseline if whittle_baseline else 'N/A'}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("BT-PPO-SRAC — Deadline Scheduling (Local Plotting)")
    print("=" * 60)
    ...
    for exp_N, exp_M in experiments:
        ...
        cfg = BT_PPOConfig(num_episodes=30000, test_every=50, log_every=50, device=device)
        ...
        agent = BT_PPO_SRAC(DeadlineSchedulingEnv(**env_kwargs), cfg)
        ...
        plt.plot(eps, rewards, label='BT-PPO-SRAC (Test Reward)', marker='o', markersize=4)
